...
# --- 2. Gelişmiş Gürkan AI Araştırmacı Motoru ---
def get_beast_analysis(symbol):
    try:
        ...
        # Zeka Katmanı: Para Akışı ve RSI
        delta = df['Close'].diff(); gain = (delta.where(delta > 0, 0)).rolling(14).mean(); loss = (-delta.where(delta < 0, 0)).rolling(14).mean()
        rsi = 100 - (100 / (1 + (gain.iloc[-1] / loss.iloc[-1])))
        
        # Gürkan AI Yorum Mantığı
        up_pot = round(((upper_b.iloc[-1] - last_p) / last_p) * 100, 1)
        down_risk = round(((last_p - lower_b.iloc[-1]) / last_p) * 100, 1)
        
        if rsi > 70: note, clr = "Aşırı şişme var, dikkatli ol patron. Kar satışı kapıda olabilir.", "#ff4b4b"
        elif rsi < 35: note, clr = "Hisse bedava noktasına yaklaşmış. Toplama alanı olabilir.", "#00ff88"
        elif last_p > ma20.iloc[-1]: note, clr = "Trend rayında. Akıllı para içeride kalmaya devam ediyor.", "#00ff88"
        else: note, clr = "Biraz keyifsiz. Destek seviyelerini (Alt Bant) beklemek mantıklı.", "#ffcc00"

        return {"p": last_p, "up": max(up_pot, 1.5), "down": down_risk, "rsi": rsi, "note": note, "color": clr, "df": df, "ch": ((last_p - df['Close'].iloc[-2]) / df['Close'].iloc[-2]) * 100}
    except: return None

# --- 3. ARAYÜZ VE STİL ---
st.set_page_config(page_title="Gürkan AI PRO v143", layout="wide", initial_sidebar_state="collapsed")
st.markdown("""<style>
    ...
    .neon-text { color: #ffcc00; font-weight: bold; }
</style>""", unsafe_allow_html=True)

# Üst Arama Paneli (Kompakt)
...
with c_fav:
    st.markdown("<p style='font-size:11px; color:#8b949e;'>LİSTE</p>", unsafe_allow_html=True)
...
with c_main:
    res = get_beast_analysis(st.session_state["last_sorgu"])
    if res:
        # Analiz Kartı
        st.markdown(f"""
        <div class='glass-card'>
            <div style='display:flex; justify-content:space-between; margin-bottom:10px;'>
                <b style='color:#ffcc00;'>🤵 GÜRKAN AI YORUMU:</b>
                <span style='color:{res['color']}; font-size:12px; font-weight:bold;'>GÜVENLİ BÖLGE</span>
            </div>
            <p style='font-size:13px; color:#e1e1e1;'>{res['note']}</p>
            <div style='display:flex; justify-content:space-around; margin-top:10px; border-top:1px solid #333; padding-top:10px;'>
                ...
                <div style='text-align:center;'> <small style='color:#8b949e;'>RSI</small><br><b>{res['rsi']:.1f}</b> </div>
            </div>
        </div>
...
with c_radar:
    st.markdown("<p style='font-size:11px; color:#8b949e;'>RADAR</p>", unsafe_allow_html=True)
