...
if check_access():
    ...
    with c_st2: 
        if st.session_state["role"] == "admin": st.warning("🛠️ MOD: ADMIN")
    ...
    with col_main:
        ...
        try:
            ...
            if not df.empty:
                ...
                df = df.tail(20) # 20 GÜNLÜK KISITLAMA
                ...
                m2.metric("TREND", "YUKARI" if fiyat > ma else "AŞAĞI")
                ...
                st.markdown(f"""
                    <div class='asistan-box'>
                        ...
                        🛡️ Zarar Kes: <span style='color:#ff4b4b;'>{stop:.2f}</span> | <b>Sinyal:</b> {'Pozitif' if fiyat > ma else 'Bekle'}
                    </div>
                """, unsafe_allow_html=True)
    ...
    with col_radar:
        ...
        for s in radar_list:
            try:
                ...
                st.markdown(f"<div class='radar-card'><div style='display:flex; flex-direction:column;'><span class='radar-name'>{h_name}</span><span class='radar-vol'>Hacim: {hacim} TL</span></div><div class='radar-pct {cls}'>%{pct:.2f}</div></div>", unsafe_allow_html=True)
            except: continue
