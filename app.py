...
def get_advanced_analysis(symbol):
    try:
        ...
        # Volatilite Tabanlı Oranlar (Zeka Modülü)
        ...
        if rsi > 70: strat, color, comment = "DOYUM NOKTASI", "#ff4b4b", "RSI aşırı alımda, kar realizasyonu beklenebilir."
        elif rsi < 35: strat, color, comment = "DİP ALIM", "#00ff88", "Hisse dipte, buradan tepki alımları gelebilir."
        elif last_p > ma20: strat, color, comment = "TREND POZİTİF", "#00ff88", f"Fiyat {ma20:.2f} üzerinde, görünüm güçlü."
        else: strat, color, comment = "ZAYIF SEYİR", "#ffcc00", "Trend desteği kırılmış, temkinli olunmalı."

        return {
            "last_p": last_p, "rsi": rsi, "up": up_pot, "down": down_risk, 
            "strat": strat, "color": color, "comment": comment, "df": df, "change": change
        }
    except: return None
...
if not st.session_state["access_granted"]:
    ...
    with t2:
        with st.form("a"):
            ...
            if st.form_submit_button("ADMİN GİRİŞİ"):
                if u == "GURKAN" and p == "HEDEF2026":
                    st.session_state["access_granted"] = True; st.rerun()
