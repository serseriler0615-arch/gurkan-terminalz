...
# Hızlı Durum Kontrolü (Radar için Cache'li)
...
def check_status(symbol):
    try:
        ...
        if rsi > 70: return "🔴" # Riskli / Şişmiş
        if rsi < 35: return "🟢" # Fırsat / Dip
        return "🟡"
    except: return "⚪"
...
if check_access():
    ...
    with col_main:
        ...
        try:
            ...
            if not df.empty:
                ...
                else: strat, color, c_txt = "ZAYIF SEYİR", "#ffcc00", "Trend desteği kırılmış, dikkatli olunmalı."
    ...
    with col_radar:
        ...
        for r in ["THYAO", "ASELS", "EREGL", "TUPRS", "AKBNK", "SISE", "BIMAS"]:
            status_icon = check_status(r)
            if st.button(f"{status_icon} {r}", key=f"rad_{r}", use_container_width=True):
