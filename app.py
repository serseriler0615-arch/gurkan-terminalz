...
if check_access():
    ...
    st.markdown("""
        ...
        .haber-card { background: #161b22; border-left: 3px solid #0088ff; padding: 10px; margin-bottom: 8px; border-radius: 6px; }
    ...
    with col_fav:
        st.markdown("### ⭐ TAKİP")
        y_fav = st.text_input("Ekle:", key="f_add_v69", label_visibility="collapsed").upper()
    ...
    with col_main:
        ...
        try:
            # Gelişmiş Veri Çekme (Bağlantı Hatasına Karşı)
            ticker = yf.Ticker(sembol)
            df = ticker.history(period="6mo", interval="1d")
            
            if df.empty:
                # Alternatif deneme (Yedek veri çekme modu)
            ...
            if not df.empty:
                ...
                if news:
                    for n in news:
                        ...
                        st.markdown(f"<div class='haber-card'><b>[{dt}]</b> {n['title']}</div>", unsafe_allow_html=True)
                else:
                    st.markdown("<div class='haber-card'>⚠️ Şirkete dair güncel haber akışı şu an sessiz.</div>", unsafe_allow_html=True)

                # VIP Analiz
                st.markdown(f"""<div class='asistan-box'><b style='color:#00ff88;'>🤵 VIP ANALİZ:</b> {h_input} için teknik güven skoru %{skor}. { 'Grafik alıcılı seyrediyor, dirençler takip edilmeli.' if skor > 60 else 'Hissede hacim ve trend onayı beklenmeli.' }</div>""", unsafe_allow_html=True)
            else:
                st.error("Veri bağlantısı başarısız. Lütfen sembolü (örn: THYAO) kontrol edin.")
    ...
    with col_radar:
        ...
        except: st.warning("Radar verisi alınamıyor.")
        ...
        if st.session_state.get("role") == "admin":
            ...
            if st.button("YENİ KEY"): st.code(f"GAI-{int(time.time())}-30-VIP")
